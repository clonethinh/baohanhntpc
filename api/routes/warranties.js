...
import { customerLabel, getCustomerRows, getWarrantyCustomerKey } from '../lib/customers.js';
...
router.get('/:id', async (req, res) => {
  try {
    const warranties = await getCollection('warranties');
    const w = warranties.find(x => x.id === req.params.id && !x.deletedAt);
    if (!w) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Không tìm thấy phiếu.' } });
    res.json({ success: true, data: withDefaultDueDate(w) });
  } catch (err) {
    res.status(500).json({ success: false, error: { code: 'SERVER_ERROR', message: 'Lỗi máy chủ, thử lại sau.' } });
  }
});
...
router.patch('/:id/customer', async (req, res) => {
  try {
    ...
    const customer = getCustomerRows(warranties).find((row) => row.key === targetKey);
    if (!customer) {
    ...
    db.warranties[wIdx] = {
      ...
      history: [
        ...(db.warranties[wIdx].history || []),
        {
          ...
          changes: { supplierIdCurrent: { from: db.warranties[wIdx].supplierIdCurrent || null, to: supplier.id } },
    ...
    db.warranties[wIdx] = {
      ...
      history: [
        ...(warranty.history || []),
        {
          ...
          changes: { supplierStatus: { from: 'sent', to: 'returned' } },
